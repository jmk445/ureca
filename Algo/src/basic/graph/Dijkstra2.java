...
//	static boolean[] visit;	// visit <== 비용 비교를 통해서 visit를 처리, 무조건 다시 가는 경우에는 비쌀 수 밖에 없음 
	...
	public static void main(String[] args) throws Exception{
		...
		while(!pqueue.isEmpty()) {
			...
			//이떄, vertex . v는 다음 갈곳, c는 그 곳까지의 최소 cost(후에 변경할거기 때문, 왜 최소인지는 뒤에 알게됨)
			//그러면, 이미 방문한 경우에는, 무조건 vertex.c
			if(cost[vertex.v] < vertex.c) continue;
			
			//이제 새로운 버텍스들 후보를 찾아봐야함. 
			for(int i = 0; i < V; i++) {
				if(matrix[vertex.v][i] == 0) continue;
				if(cost[i] > matrix[vertex.v][i] + vertex.c) {
					cost[i] = matrix[vertex.v][i] + vertex.c;
					//자이제 그 후보까지의 정보를 나타내는 vertex는 합격..-> 큐에 넣어줌
